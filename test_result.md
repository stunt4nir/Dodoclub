...
backend:
  ...
  - task: "Tournament expansion + manual scheduling (POST /api/tournaments/{tid}/matches, double round-robin, all-same-date, custom kick-off time)"
    implemented: true
    working: true
    file: "backend/server.py"
    stuck_count: 0
    ...
    status_history:
        - working: "NA"
          agent: "main"
          comment: "Major tournament scheduling extension. (1) TournamentCreateIn now accepts: start_time (HH:MM, default 19:00), all_same_date (bool, when true ALL fixtures are scheduled on start_date with hourly increments), double_round_robin (bool, when true each pair plays twice — return-leg appended). (2) NEW endpoint POST /api/tournaments/{tid}/matches (admin-only) appends a custom fixture: body {home, away, scheduled_at (full ISO datetime)}. Validates home/away ∈ team_names and home != away. Auto-pre-populates lineup from the tournament's stored team_rosters. Increments fixtures.round to len(fixtures)+1. (3) NEW endpoint PATCH /api/matches/{mid}/datetime (editor) updates only `date` field; accepts free-form ISO 8601 datetime — no 7-day cap. (4) NEW endpoint POST /api/matches/{mid}/finish (editor) finalises the match using current score_a/b/c with empty stats; sets status='played'; awards W/D/L and league_points based on team outcomes (mirroring /result logic). Refuses if already 'played' (400). (5) NEW endpoint POST /api/matches/{mid}/lineup/positions (editor) accepts {positions: {user_id: {x, y}}} and persists per-player x/y overrides on the lineup's player objects (for drag-to-reposition). Need to verify: (a) double_round_robin produces 2*C(n,2) fixtures with home/away swapped on the second leg; (b) all_same_date schedules all fixtures on start_date with hourly bumps; (c) start_time HH:MM honoured; (d) add-match endpoint creates a new fixture with auto-roster from tournament.team_rosters and increments round count, validation 400 on unknown team / same team; (e) datetime PATCH accepts any future ISO datetime; (f) finish endpoint awards league points & W/L/D using current score_a/b, sets status=played; (g) /finish refuses on already played (400); (h) lineup/positions persists x/y on player objects, returns updated match with x/y in lineup arrays."
        - working: true
          agent: "testing"
          comment: "66/66 PASS in /app/backend_test.py — all 5 new/extended pieces of behavior verified end-to-end against the live backend. Setup: admin login (admin@clubdodo.com/dodo2026).\n\n• (A) DOUBLE ROUND-ROBIN: POST /api/tournaments {team_names:[Red,Black,White], team_size:5, double_round_robin:true} → 200 with fixtures.length==6. First 3 pairings covered each pair once; second 3 pairings had home/away swapped vs the first 3 (verified by equality of second-leg list vs reversed first-leg list). DELETE cascade confirmed (GET on tid returns 404 afterwards).\n\n• (A) ALL_SAME_DATE + START_TIME: POST with start_date=2026-07-01, start_time=20:30, all_same_date=true → 3 fixtures all on 2026-07-01 with hourly bumps: fixture[0].scheduled_at prefixed '2026-07-01T20:30', fixture[1] '2026-07-01T21:30', fixture[2] '2026-07-01T22:30'. DELETE ok.\n\n• (B) ADD MATCH POST /api/tournaments/{tid}/matches: Created tournament with team_rosters {Red:[u1,u2], Black:[u3,u4], White:[u5,u6]} (6 fresh users). POST {home:'Red', away:'Black', scheduled_at:'2026-08-15T18:00:00+00:00'} → 200; returned tournament has fixtures.length==4, new fixture has round==4, home=Red, away=Black. GET /api/matches/{new_mid} → title 'Add Match Cup ...: Red vs Black', status='scheduled', lineup.team_a exactly == Red roster (2 uids), lineup.team_b exactly == Black roster (2 uids), 4 total votes all set to 'yes'. Validation: {home:'Red', away:'Red'} → 400 with detail containing 'different'. {home:'Yellow', away:'Black'} → 400 with detail 'Home/away must be one of the tournament teams'. {scheduled_at:'not-a-date'} → 400 'scheduled_at must be a valid ISO datetime'. Non-admin regular user → 403 'Admin only'.\n\n• (C) PATCH /api/matches/{mid}/datetime: Created match with date=2026-09-01T19:00:00+00:00. PATCH {scheduled_at:'2027-12-31T23:45:00+00:00'} → 200; GET match.date contains '2027-12-31T23:45'. Bad ISO 'not-a-date' → 400. Non-editor → 403.\n\n• (D) POST /api/matches/{mid}/finish: Created league match team_size=5, PUT lineup with 2 users on each team (team_a=[u1,u2], team_b=[u3,u4]), POST live-score {2,0}. POST /finish → 200, status='played', result.team_a_score=2, result.team_b_score=0, result.stats=[]. Verified stat deltas via GET /api/users: team_a user matches_played +=1, wins +=1, league_points +=3; team_b user matches_played +=1, losses +=1, league_points unchanged. Second /finish → 400 with detail 'Match already finalised'. Friendly draw scenario (team_size=5, live-score 1-1) → /finish success; team_a user matches_played +=1, draws +=1, league_points UNCHANGED (friendly — no points).\n\n• (E) POST /api/matches/{mid}/lineup/positions: Created match with lineup (team_a/team_b 2 players each). POST {positions:{u1:{x:0.42, y:0.69}}} → 200; GET lineup.team_a, target player has x=0.42 and y=0.69, other player (finish_uids[1]) has NO x/y fields (unchanged). POST {positions:{u1:{x:1.5, y:0.5}}} → 422 (pydantic le=1.0 enforced). Non-editor → 403.\n\n• (7) CLEANUP: deleted all created tournaments, matches, and 11 test users.\n\nNo backend errors. Nothing to fix — main agent can summarise and finish."

  - task: "Live in-match scoring (POST /api/matches/{mid}/live-score) + tournament standings reflect live scores"
    implemented: true
    working: true
    file: "backend/server.py"
    stuck_count: 0
    ...
    status_history:
        - working: "NA"
          agent: "main"
          comment: "Added two backend pieces. (1) New endpoint POST /api/matches/{mid}/live-score (require_editor) accepts {team_a_score, team_b_score, team_c_score?} and updates only score_a/b/c on the match document. Sets status='in_progress' the first time any goal is recorded. Refuses if status='played' (must edit final result instead). Does NOT touch player goals/assists, league_points, matches_played — purely an in-flight score broadcast. (2) Updated _tournament_public to compute standings using LIVE scores (match.score_a/score_b) when no final result exists. New per-fixture flag `live: true` is set when match has running scores or status='in_progress' but no result. Standings W/D/L/GF/GA/GD/Pts now include live matches (3 pts for current leader, 1 each for tied). When a final result is later recorded, _tournament_public switches to using result.team_a_score/team_b_score (authoritative). Need to verify: (a) POST /live-score updates score_a/b on the match (unauth → 401, non-editor → 403); (b) status flips to 'in_progress' on first goal; (c) GET /api/tournaments/{tid} reflects live scores in fixtures (live=true) and standings; (d) recording final result with POST /matches/{mid}/result still works and standings switch to using result.team_a/b_score (live=false, played=true); (e) cannot call /live-score on a played match (400)."
        - working: true
          agent: "testing"
          comment: "60/60 PASS in /app/backend_test.py — all 5 review-request sections (1 endpoint basics, 2 finalised guard, 3 tournament standings with live scores, 4 purity, 5 cleanup) verified end-to-end against the live backend. Setup: admin login (admin@clubdodo.com/dodo2026), registered 6 fresh users (livetest_<suf>_1..6) with varied positions (CB/LB/CDM/CAM/LW/ST), created a 3-team tournament (Red/Black/White, team_size=5, 2 players each) yielding 3 round-robin fixtures.\n\n• S1 ENDPOINT BASICS (12 checks): Initial GET /api/tournaments/{tid} → all fixtures live=false, played=false, standings all zeros. POST /live-score without Authorization → 401. As regular non-editor user → 403. Fixture[0] match starts as status='scheduled'. POST {team_a_score:0, team_b_score:0} as admin → 200; response status still 'scheduled'; tournament GET confirms fixture not counted in standings (all P=0). POST {1,0} → 200, response status='in_progress'; tournament GET shows fixture[0] live=true, played=false, score_home=1, score_away=0; home team standings P=1 W=1 GF=1 GA=0 GD=1 Pts=3; away team P=1 L=1 GF=0 GA=1 GD=-1 Pts=0; third team all zeros. POST {1,2} → 200, status still 'in_progress', score 1-2 via tournament. POST {100,0} → 422 (le=99). POST {-1,0} → 422 (ge=0).\n\n• S2 CANNOT UPDATE FINALISED MATCH (4 checks): POST /matches/{m1}/result {team_a_score:3, team_b_score:1, stats:[]} → 200, status='played'. Subsequent POST /live-score → 400 with detail 'Match already finalised — edit the result instead' (contains 'finalis'/'already' as expected).\n\n• S3 TOURNAMENT STANDINGS WITH LIVE SCORES (15 checks): Live 1-1 tied draw on m1 → fixture live=true, both teams P=1 D=1 GF=1 GA=1 GD=0 Pts=1 (correctly 1 pt each for tied live). After finalising 3-1: fixture[0] played=true live=false score_home=3 score_away=1; home P=1 W=1 GF=3 GA=1 GD=2 Pts=3; away P=1 L=1 GF=1 GA=3 GD=-2 Pts=0 (confirms flip from live to authoritative). Bonus: live 0-2 on fixture[1] (m2) → fixture[1] live=true played=false score 0-2 while fixture[0] remains played=true. Combined standings dynamically recomputed from fixtures and matched backend standings exactly across all teams (P/W/D/L/GF/GA/Pts).\n\n• S4 PURITY (7 checks): Captured admin GET /auth/me baseline (goals/assists/matches_played/league_points/rating/wins/draws/losses). Fired 3 more /live-score bumps ({2,2},{3,2},{3,3}). Post-bump GET /auth/me identical to baseline (no stat mutation). Also logged in as 2 rostered players (Red team) — their goals/assists/matches_played/league_points still all 0 after live-score activity on their match.\n\n• S5 CLEANUP (9 checks): DELETE /tournaments/{tid} → 200; both fixture match_ids subsequently 404 (cascade delete confirmed). All 6 test users DELETE /users/{uid} → 200. No leakage into admin or squad.\n\nSAMPLE REQUEST/RESPONSE:\n  POST /api/matches/{m1}/live-score  body={\"team_a_score\":1,\"team_b_score\":0}  Authorization: Bearer <admin-jwt>\n    → 200  {\"id\":\"5ee28d43-...\", \"title\":\"Live Cup ...: Black vs White\", \"status\":\"in_progress\", \"team_size\":5, \"votes\":[...4 yes...], \"lineup\":{...home+away rosters...}, \"result\":null, ...}\n  GET /api/tournaments/{tid}\n    → fixtures[0] = {\"match_id\":\"5ee28d43-...\", \"home\":\"Black\", \"away\":\"White\", \"round\":1, \"scheduled_at\":\"...T19:00:00+00:00\", \"played\":false, \"live\":true, \"score_home\":1, \"score_away\":0, \"scorers\":[], \"assisters\":[]}\n    → standings = [{team:'Black',P:1,W:1,D:0,L:0,GF:1,GA:0,GD:1,Pts:3}, {team:'Red',P:0,...,Pts:0}, {team:'White',P:1,W:0,D:0,L:1,GF:0,GA:1,GD:-1,Pts:0}]\n\nMINOR (not flagged as failure): _match_public() does NOT expose the raw score_a/score_b/score_c fields in its response payload (only status is visible). The spec line 'score_a/score_b/score_c==0' in the request is verified indirectly via the tournament endpoint's fixture.score_home/score_away, which are computed from the stored match.score_a/score_b. If the UI needs the raw scores directly on the match object, main agent may consider adding them to _match_public — but all tournament-driven standings logic works correctly. Nothing blocking. No backend errors in /var/log/supervisor/backend.err.log during the run."
        - working: true
          agent: "main"
          comment: "Addressed the minor observation: _match_public now exposes score_a, score_b, score_c (defaults 0) AND tournament_id, tournament_home, tournament_away. Backend reloaded cleanly; existing tests unaffected as these are purely additive response keys. Frontend match screen + matches list can now read raw live scores directly."
...
test_plan:
  current_focus:
    - "Tournament expansion + manual scheduling (POST /api/tournaments/{tid}/matches, double round-robin, all-same-date, custom kick-off time)"
  stuck_tasks: []
  test_all: false
...
agent_communication:
    ...
    - agent: "testing"
      message: "Tournament expansion + manual scheduling — 66/66 PASS in /app/backend_test.py. All 5 new/extended behaviors verified: (A) double_round_robin=true produces 2×C(n,2)=6 fixtures with second-leg home/away swapped; all_same_date=true with start_time=20:30 schedules 3 fixtures on 2026-07-01 at 20:30/21:30/22:30 (hourly bumps). (B) POST /api/tournaments/{tid}/matches — admin-only (non-admin 403), validates home!=away (400 'different'), home/away ∈ team_names (400 'Home/away must be one of'), ISO datetime (400 'valid ISO'); returns tournament with fixtures+1, new round=fixtures+1, match lineup pre-populated from team_rosters with votes='yes' for all rostered players. (C) PATCH /api/matches/{mid}/datetime — accepts any ISO datetime (no 7-day cap), persists date, 400 on bad ISO, 403 for non-editor. (D) POST /api/matches/{mid}/finish — finalises with empty stats, status='played', result.team_a_score/team_b_score match current live score; league match awards +3 league_points+wins+matches_played to team_a (won 2-0), matches_played+losses to team_b with league_points unchanged; second call 400 'already finalised'; friendly 1-1 draw → matches_played +=1 and draws +=1 but league_points UNCHANGED. (E) POST /api/matches/{mid}/lineup/positions — persists x/y on target player object, other players untouched (no x/y fields added), 422 on x>1.0 (pydantic le=1.0), 403 for non-editor. All test data (tournaments, matches, users) cleaned up. No backend errors. Nothing to fix — main agent can summarise and finish."
    - agent: "testing"
      message: "Live in-match scoring + tournament live standings — 60/60 PASS in /app/backend_test.py. All review-request scenarios verified against live backend: (1) Endpoint basics — unauth→401, non-editor→403, admin 0-0 on scheduled match leaves status='scheduled' and does NOT contribute to standings; 1-0 flips status to 'in_progress', fixture.live=true/played=false, standings reflect 3pts to home; 1-2 updates correctly; team_a_score=100→422 (le=99); -1→422 (ge=0). (2) Finalised guard — after POST /matches/{mid}/result, subsequent /live-score returns 400 'Match already finalised — edit the result instead'. (3) Tournament standings — live 1-1 correctly marks both teams 1 pt each (tied draw); recording final result 3-1 flips fixture from live→played (live=false, played=true, score_home=3, score_away=1) and standings refresh authoritatively from result.team_a/b_score; live on fixture[1] coexists correctly with played fixture[0] (combined standings dynamically recomputed and matched). (4) Purity — admin goals/assists/matches_played/league_points/rating/wins/draws/losses UNCHANGED across multiple /live-score bumps; rostered players' stats also untouched. (5) Cleanup — DELETE tournament cascades fixtures (404), all 6 test users deleted. MINOR (not a failure): _match_public() does not expose raw score_a/score_b/score_c on the match payload — verification was done via the tournament endpoint's fixture.score_home/score_away which is authoritative. If the frontend needs raw scores on the match object directly, main agent could add them to _match_public. No backend errors. Nothing to fix — main agent can summarise and finish."
